...
async function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 1586, height: 992, minWidth: 1280, minHeight: 760,
    backgroundColor: '#071016', title: 'ForgeCAD — AI Engineering Studio', show: false,
    webPreferences: { preload: path.join(__dirname, 'preload.mjs'), contextIsolation: true, nodeIntegration: false, sandbox: true },
  });
  mainWindow.once('ready-to-show', () => mainWindow?.show());
  if (VITE_DEV_SERVER_URL) await mainWindow.loadURL(VITE_DEV_SERVER_URL); else await mainWindow.loadFile(path.join(RENDERER_DIST, 'index.html'));
}
...
function createMenu() {
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    { label: 'Project', submenu: [] }, { label: 'Simulation', submenu: [] }, { label: 'Tools', submenu: [] }, { role: 'windowMenu' }, { role: 'help' },
  ];
...
app.whenReady().then(async () => {
  ...
  try { await createMainWindow(); }
    ...
    await dialog.showMessageBox({ type: 'error', title: 'ForgeCAD could not start', message: 'Forge Engine failed its startup gate.', detail, buttons: ['Quit'] });
    app.quit(); return;
  }
