...
function metric(value: unknown, unit = '', digits = 2) { const number = finiteNumber(value); return number == null ? '—' : `${number.toFixed(digits)}${unit}`; }
...
export function EngineeringWorkbench({ mode, project, selectedId, activeJob, onProject, onStartSimulation, onStartCampaign, onJobStarted }: Props) {
  ...
  async function status(status: 'working' | 'not_working' | 'unverified') {
    if (!activeBranch) return;
    try {
      setBusy(true);
      const result = await setBranchStatus(activeBranch.name, status, 'Set from ForgeCAD design inspector. Physical verification requires recorded evidence.', false);
  ...
  async function compare() {
    ...
    try { setBusy(true); setComparison(await compareBranch(workingBranch.name)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(false); }
  }
  async function activate(name: string) {
    try { setBusy(true); setError(null); onProject(await activateBranch(name)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(false); }
  }
  ...
  async function onStep(file?: File) {
    if (!file) return;
    try { setBusy(true); onProject((await importStepFile(file)).project); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(false); if (stepInput.current) stepInput.current.value = ''; }
  }
  async function onBundle(file?: File) {
    if (!file) return;
    try { setBusy(true); onProject((await importProjectBundle(file)).project); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
  ...
  async function startConfiguredCampaign() {
    ...
    const payload: Record<string, unknown> = { objective: campaignObjective, process: campaignProcess, force_n: force, deflection_max_mm: deflection, yield_fos_min: fos, max_candidates: count };
    ...
    try {
      setError(null);
      const job = await createJob({ kind: 'campaign', selected_object_id: campaignTarget.id, branch: project.active_branch, payload });
      onJobStarted?.(job);
    } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
  }
  ...
  if (mode === 'design') return <div className="component-library" data-testid="design-inspector">
    ...
    <div className="campaign-card">
      ...
      <div className="filter-row">
        <button disabled={busy} onClick={() => void status('working')}>Mark working</button>
        <button disabled={busy} onClick={() => void status('not_working')}>Mark not working</button>
        <button disabled={busy} onClick={() => void status('unverified')}>Unverified</button>
      </div>
      <p className="campaign-disclaimer"><ShieldAlert size={11}/>Branch status is a design label. Physical verification is granted only by revision-bound recorded test evidence.</p>
    ...
    {selected && <div className="campaign-card"><div className="campaign-title"><PackageOpen size={18}/><div><strong>{selected.name}</strong><span>{selected.role} · {selected.geometry_fidelity ?? 'geometry'} · {selected.mass_g} g</span></div></div><p>{selected.component_ref ? `Registry component: ${selected.component_ref}` : 'Fabricated/custom part generated from exact CAD geometry.'}</p></div>}
    <EngineeringDomainSummary validation={validation} project={project} mode="design"/>
    <div className="campaign-card"><strong>Portable project + CAD import</strong><p><code>.focad</code> bundles carry project state, frozen component snapshots, code and local CAD assets.</p><div className="filter-row"><button disabled={busy} onClick={() => void exportBundle()}><Download size={13}/> Export .focad</button><button disabled={busy} onClick={() => bundleInput.current?.click()}><Upload size={13}/> Import .focad</button><button disabled={busy} data-testid="import-step" onClick={() => stepInput.current?.click()}><FileUp size={13}/> Import STEP</button></div><input ref={bundleInput} hidden type="file" accept=".focad,.zip,.forgecad.zip" onChange={(event) => void onBundle(event.target.files?.[0])}/><input ref={stepInput} hidden type="file" accept=".step,.stp" onChange={(event) => void onStep(event.target.files?.[0])}/></div>
    <div className="campaign-card"><strong>Bill of materials</strong><p>{project?.bom?.length ?? 0} purchased line items · {project?.connections?.length ?? 0} modeled interface connections.</p>{(project?.bom ?? []).slice(0, 8).map((item, index) => <div className="history-row" key={index}><strong>{String(item.manufacturer ?? '')} {String(item.model ?? item.description ?? '')}</strong><small>qty {String(item.qty ?? 1)} · {String(item.supplier ?? 'catalog')}</small></div>)}</div>
  </div>;
  ...
  return <div className="component-library engineering-analysis" data-testid="analysis-workspace">
    ...
    <div className="campaign-card"><div className="campaign-title">{validation?.ok ? <CheckCircle2 size={18}/> : <ShieldAlert size={18}/>}<div><strong>{validation?.ok ? 'Reality checks pass' : 'Engineering risks need attention'}</strong><span>{validation ? `${validation.counts.error} errors · ${validation.counts.warning} warnings · ${validation.counts.info} info` : 'Loading validation…'}</span></div></div><button disabled={busy} onClick={() => void refresh()}><RefreshCw size={13}/> Re-run validation</button>{(validation?.risks ?? []).slice(0, 8).map((risk, index) => <div className="history-row" key={`${risk.code}-${index}`}><span>{risk.severity}</span><strong>{risk.message}</strong><small>{risk.code ?? 'engineering_check'}</small></div>)}</div>
    <EngineeringDomainSummary validation={validation} project={project}/>
    {requirements.length > 0 && <div className="campaign-card requirement-gates" data-testid="requirement-gates"><strong>Requirement gates</strong><p>Deterministic metrics and recorded physical evidence are evaluated separately; evidence cannot override a failing numeric gate.</p><div className="requirement-list">{requirements.slice(0, 10).map((row, index) => {
      const passed = row.passed === true; const failed = row.passed === false; const statusText = passed ? 'PASS' : failed ? 'FAIL' : String(row.verification_status ?? 'UNVERIFIED').toUpperCase();
      return <div className="requirement-row" key={String(row.id ?? index)}><span className={`requirement-state ${passed ? 'good' : failed ? 'bad' : ''}`}>{statusText}</span><div><strong>{String(row.statement ?? row.description ?? row.id ?? 'Requirement')}</strong><small>{row.value != null ? `Measured ${String(row.value)} against ${String(row.op ?? '')} ${String(row.target ?? '')}` : String(row.verification_method ?? row.verification ?? 'Verification pending')}</small></div></div>;
    })}</div></div>}
    ...
    {campaignResult && <div className="campaign-card campaign-results" data-testid="campaign-results">
      ...
      <div className="candidate-list">{(campaignResult.candidates ?? []).map((row, index) => {
        const displacement = row.structural?.max_displacement_mm ?? row.structural?.max_deflection_mm; const winner = row.branch === campaignResult.winner_branch; const active = project?.active_branch === row.branch; const failedGates = Object.entries(row.verifier?.gates ?? {}).filter(([, passed]) => !passed).map(([gate]) => gate);
        return <div className={`candidate-row ${row.feasible ? 'feasible' : 'rejected'} ${winner ? 'winner' : ''}`} key={row.branch}><div className="candidate-rank">{winner ? <Trophy size={12}/> : index + 1}</div><div className="candidate-main"><strong>{row.branch}</strong><small>{parameterSummary(row.parameters)}</small><div className="candidate-metrics"><span>mass {metric(row.metrics?.mass_kg, ' kg', 3)}</span><span>defl. {metric(displacement, ' mm', 3)}</span><span>FoS {metric(row.structural?.yield_fos, '', 2)}</span><span>temp {metric(row.thermal?.max_temperature_c, ' °C', 1)}</span></div>{row.error && <em>{row.error}</em>}{!row.feasible && failedGates.length > 0 && <em>Failed: {failedGates.join(', ')}</em>}</div><div className="candidate-side"><span className={`candidate-state ${row.feasible ? 'good' : 'bad'}`}>{row.feasible ? winner ? 'WINNER' : 'PASS' : 'REJECT'}</span><button disabled={busy || active} onClick={() => void activate(row.branch)}>{active ? 'Active' : 'Open'}</button></div></div>;
      })}</div><div className="campaign-disclaimer"><ShieldAlert size={11}/>Campaign passes are deterministic screening results. The selected branch remains unverified until real-world evidence is recorded and the design is explicitly marked working.</div>
    </div>}
    <div className="campaign-card"><strong>Real component registry</strong><p>{stats ? `${stats.total} total components · ${stats.builtin} built in · ${stats.categories.length} categories` : 'Loading registry statistics…'}</p>{stats && <p>Geometry fidelity: {Object.entries(stats.geometry_fidelity).map(([key, value]) => `${key} ${value}`).join(' · ')}</p>}</div>
  </div>;
}
