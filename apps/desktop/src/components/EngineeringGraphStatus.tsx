...
export function EngineeringGraphStatus({ selectedObjectId = null }: { selectedObjectId?: string | null }) {
  ...
  return <div data-testid="engineering-graph-status" style={{ borderTop: divider, paddingTop: 8, marginTop: 1 }}>
    ...
    {error ? <div style={{ fontSize: 9.5, color: 'var(--warning, #f1bf55)', marginBottom: 5 }}>{error}</div> : null}
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
      ...
      <TinyMetric label="DIRTY" value={health?.engineering_graph.dirty_node_count ?? '—'} warn={(health?.engineering_graph.dirty_node_count ?? 0) > 0}/>
      <TinyMetric label="STALE EV." value={health?.engineering_graph.stale_evidence_count ?? '—'} warn={(health?.engineering_graph.stale_evidence_count ?? 0) > 0}/>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: 5, fontSize: 9.5, marginTop: 6 }}>
      ...
      <span style={{ opacity: .5 }}>Sync</span><strong style={{ color: health?.graph_sync.ok ? 'var(--accent, #55f59a)' : 'var(--warning, #f1bf55)' }}>{health?.graph_sync.ok ? 'Current' : 'Needs attention'}</strong>
    </div>
    ...
    <div style={{ borderTop: divider, paddingTop: 7, marginTop: 7 }} data-testid="product-lab-profile">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
        <ModeButton active={mode === 'general_engineering'} disabled={busy} onClick={() => void setMode('general_engineering')}>General</ModeButton>
        <ModeButton active={mode === 'product_lab'} disabled={busy} onClick={() => void setMode('product_lab')}>Product Lab</ModeButton>
      </div>
      {mode === 'product_lab' ? <div style={{ fontSize: 9, opacity: .65, lineHeight: 1.35, marginTop: 5 }}>
...
function ModeButton({ active, disabled, onClick, children }: { active: boolean; disabled: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button disabled={disabled} onClick={onClick} style={{ border: active ? '1px solid var(--accent, #55f59a)' : divider, borderRadius: 4, background: active ? 'var(--accent-dim, rgba(85,245,154,.12))' : 'transparent', color: 'inherit', fontSize: 9, padding: '4px 5px', cursor: 'pointer' }}>{children}</button>;
}
