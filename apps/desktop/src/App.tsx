...
export default function App() {
  ...
  return <main className="forge-shell">
    ...
    <section className="workspace">
      ...
      <section className="lineage-strip"><span className="lineage-label">Design lineage⌄</span><div className="lineage-flow">{project?.branches.map((branch, index) => <div className="lineage-item" key={branch.name}><BranchCard branch={branch}/>{index < (project?.branches.length ?? 0) - 1 && <span className="lineage-arrow">→</span>}</div>)}</div><select><option>Design status: All</option></select><label className="verified-toggle"><input type="checkbox" defaultChecked/>Works in real life</label><button className="toolbar-button">Compare to working</button></section>
