...
"""Validate the capabilities that must survive ForgeCAD 6.0 native packaging.

This intentionally talks to a running packaged Forge Engine over its public HTTP
surface. It proves that the bundled runtime exposes the 6.0 release identity and can
load/run a packaged Cantera mechanism, not merely that the executable starts.
"""
...
def validate(base_url: str, token: str, expected_release_complete: bool, timeout_s: float) -> dict[str, Any]:
    ...
    with httpx.Client(base_url=base_url.rstrip("/"), timeout=15.0) as client:
        while time.monotonic() < deadline:
            ...
            time.sleep(0.25)
        if health is None:
            raise RuntimeError(f"ForgeCAD 6.0 health never became ready at {base_url}")

        assert health["api_version"] == "6.0", health
        assert health["engine_version"] == "6.0.0", health
