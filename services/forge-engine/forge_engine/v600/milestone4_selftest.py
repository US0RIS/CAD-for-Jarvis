...
def run() -> dict[str, object]:
    ...
    with TestClient(app) as client:
        ...
        pi = next(row for row in core.PROJECT["objects"] if row.get("component_ref") == "compute.raspberry_pi_5_8gb")
        pi_id = str(pi["id"])
        ...
        result = _ok(client.post("/v6/engineering/autonomous-repair", json=payload), "run milestone 4 bounded autonomous repair").json()
        ...
        # The rejected parametric branch is retained and records why the solid
        # solver could not validate its featured geometry.
        param_outcome = parametric["outcome"]
        assert param_outcome is not None and param_outcome["ok"] is False, parametric
        reasons = [item.get("reason", "") for evaluation in param_outcome["evaluations"] for item in evaluation["analysis_refresh"]["items"] if not item["ok"]]
        ...
        # Baseline still contains the original purchased component and geometry.
