...
The substitution fixture uses two actual catalog NEMA-17 motors with matching
mount/shaft/power interface contracts. This avoids weakening compatibility simply
to make a lighter component look interchangeable.
"""
...
def run() -> dict[str, object]:
    ...
    with TestClient(app) as client:
        ...
        add_motor = _ok(client.post("/v2/components/motor.nema17.48/add"), "ingest NEMA17 48 mm fixture").json()
        motor_id = str(add_motor["component"]["instance_id"])
        motor = core.object_by_id(motor_id)
        assert motor["component_ref"] == "motor.nema17.48", motor
        ...
        # Catalog mass drops from 480 g to 350 g; require a material 100 g
        # reduction so the substitution genuinely matters, but retain margin.
        required_reduction_kg = 0.100
        _req(client, target_id, "Reduce product mass by at least 100 grams", "mass_kg", "<=", baseline_mass - required_reduction_kg, [motor_id, block_id])
        _req(client, guard_id, "Do not add or remove canonical objects", "object_count", "==", baseline_count, [motor_id, block_id])
        ...
        result = _ok(client.post("/v6/engineering/autonomous-repair", json={
            ...
            "strategies": [
                ...
                {
                    "id": "shorter-compatible-nema17",
                    "kind": "component_substitution",
                    "change_cost": 0.2,
                    "object_id": motor_id,
                    "allowed_component_ids": ["motor.nema17.40"],
                    "actor": "jarvis"
                }
            ]
        ...
        assert result["selected"]["strategy_id"] == "shorter-compatible-nema17", result
        ...
        substitution = next(row for row in result["candidates"] if row["strategy_id"] == "shorter-compatible-nema17")
        assert parametric["accepted"] is False, parametric
        assert substitution["accepted"] is True, substitution
        assert substitution["change"]["old_component_id"] == "motor.nema17.48", substitution
        assert substitution["change"]["new_component_id"] == "motor.nema17.40", substitution
        assert core.object_by_id(motor_id)["component_ref"] == "motor.nema17.40"
        assert float(core.project_metrics()["object_count"]) == baseline_count
        assert float(core.project_metrics()["mass_kg"]) <= baseline_mass - required_reduction_kg
        ...
        assert core.object_by_id(motor_id)["component_ref"] == "motor.nema17.48"
        ...
        return {
            ...
            "compatible_catalog_substitution_selected": True,
